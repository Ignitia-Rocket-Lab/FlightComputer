...
bool alturaAlcanzada = false; // variable global
...
// Despliegue de paracaidas con Giroscopio 
void desplegarParacaidasPorDesvio() {
  Serial.println("Despliegue por desvío detectado: desplegando paracaídas DROGUE");
}
...
void loop() {
  ...
  if (gps.location.isUpdated() && hasQuaternion) {
    ...
    String hora = "NA";
    if (gps.time.isValid()) {
      int horaLocal = gps.time.hour() - 6;  // Ajuste horario
    ...
    // Filtro IIR para altura
    const float alpha = 0.5;
    if (primeraLectura) {
      altura_filtrada = altura_rel;
    ...
    if (tiempoLanzamiento == 0 && altura_filtrada > 5) {
      tiempoLanzamiento = millis();
    }

    // Leer aceleraciones IMU
    VectorInt16 aa;
    mpu.getAcceleration(&aa.x, &aa.y, &aa.z);
    float accX = aa.x / 16384.0;
    float accY = aa.y / 16384.0;
    float accZ = aa.z / 16384.0;

    // Filtrado IIR aceleración Z
    const float alphaAcc = 0.2;
    accZ_filtrada = alphaAcc * accZ + (1 - alphaAcc) * accZ_filtrada;

    // Detectar apogeo y desplegar paracaídas normal
    ...
    // NUEVO: Detectar desvío e imprimir despliegue dummy
    float magnitudDesvio = sqrt(accX * accX + accY * accY);
    const float umbralDesvio = 0.4;

    if (magnitudDesvio > umbralDesvio && !paracaidasDesplegado) {
      desplegarParacaidasPorDesvio();
      paracaidasDesplegado = true;
    }
  }

  // Verificar si ya alcanzamos ~200 m para mensaje
  if (tiempoLanzamiento > 0 && !alturaAlcanzada && (p_inicial - presion) >= 2390) {
    Serial.println("Altura alcanzada");
    alturaAlcanzada = true;
  }
}
